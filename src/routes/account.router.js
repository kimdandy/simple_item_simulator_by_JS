...
// 계정 생성 
 router.post('/account/sign_up', async(req, res, next) => {
    ...
    else if (!userId || !password || !confirm_password || !name) {
...
router.post('/account/sign_in', async (req, res, next) => {
    ...
    const token = jwt.sign(
        ...
        //'custom-secret-key', //ACCESS_TOKEN_SECRET_KEY
        process.env.ACCESS_TOKEN_SECRET,
        {   expiresIn: '1h'    } // Access Token 유효기간 설정 ; 1시간
    );
    ...
    return res.status(200).json({ message: '로그인 성공', token: token});
        ...
        console.error(error);
        return res.status(500).json({ messagee: 'Internal Server Error' });
    }
}); 
